...
let Nestjs = require('../model/nestjs');
...
const deleteNestjs = async (filter) =>{
  try {
    let response  = await dbService.deleteMany(Nestjs,filter);
    return response;
  } catch (error){
...
const deleteUser = async (filter) =>{
  try {
    ...
    if (user && user.length){
      ...
      const nestjsFilter = { $or: [{ addedBy : { $in : user } },{ updatedBy : { $in : user } }] };
      const nestjsCnt = await dbService.deleteMany(Nestjs,nestjsFilter);
...
const countNestjs = async (filter) =>{
  try {
    const nestjsCnt =  await dbService.count(Nestjs,filter);
    return { nestjs : nestjsCnt };
  } catch (error){
    throw new Error(error.message);
  }
};
...
const countUser = async (filter) =>{
  try {
    ...
    if (user && user.length){
      ...
      const nestjsFilter = { $or: [{ addedBy : { $in : user } },{ updatedBy : { $in : user } }] };
      const nestjsCnt =  await dbService.count(Nestjs,nestjsFilter);
...
const softDeleteNestjs = async (filter,updateBody) =>{  
  try {
    const nestjsCnt =  await dbService.updateMany(Nestjs,filter);
    return { nestjs : nestjsCnt };
  } catch (error){
    throw new Error(error.message);
  }
...
const softDeleteUser = async (filter,updateBody) =>{  
  try {
    ...
    if (user.length){
      ...
      const nestjsFilter = { '$or': [{ addedBy : { '$in' : user } },{ updatedBy : { '$in' : user } }] };
      const nestjsCnt = await dbService.updateMany(Nestjs,nestjsFilter,updateBody);
